...
const VideoList = () => {
  ...
  const GetPendingVideoStatus = (pendingVideo) => {
    // we find every 5 sec our video is ready or not
    ...
    <div>
      {videoList?.length == 0 ? (
        <div
         ...
         gap-5 p-5 border border-dashed rounded-xl py-16 "
        >
          ...
          {videoList?.map((video, index) => {
            <div key={index} className="relative">
              {video?.status == "completed" ? (
                ...
                <div
                  className="aspect-[2/3] p-5 w-full rounded-xl
                 bg-slate-800 flex items-center justify-center gap-2"
                >
